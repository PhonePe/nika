...
"""
    ...
    def run_tainted_variable_flow_analysis(
        self,
        pairs,
        source_annotations=None,
        request_accessors=None,
        ...
        if ping_result.get("success") is False:
            raise AstrailEngineError(
                "Astrail server is not reachable for tainted-variable flow analysis."
            )

        query_file = self._query_file_path("taintedVariableFlow.scala")
        if not os.path.exists(query_file):
            raise AstrailEngineError(
                f"Tainted-variable flow query file not found: {query_file}"
            )
        ...
        try:
            ...
            output_escaped = output_tmp.replace("\\", "\\\\")
            script = query_content + (
                f'\nfindTaintedVariableFlows("{params_escaped}", "{output_escaped}")'
            )
            result = self._execute_query_sync(script, timeout=1800)

            if result.get("success") is False:
                raise AstrailEngineError(
                    f"Tainted-variable flow query failed: {result.get('error', 'unknown')}"
                )
            ...
            raise AstrailEngineError(
                f"Error in tainted-variable flow analysis: {exc}"
            ) from exc
        finally:
            for tmp in [params_tmp, output_tmp]:
                if tmp and os.path.exists(tmp):
